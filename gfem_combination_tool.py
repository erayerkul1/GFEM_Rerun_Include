...
def read_case_ids_from_excel(excel_path: str) -> set[int]:
...
def find_bdf_files_for_ids(
    ...
    if log_fn:
        log_fn(f"Scanned {total_scanned} BDF files in search directory.")
    return result
...
def write_output_bdf(
    ...
    lines.append(f"$ Generated by GFEM BDF Combination Tool")
    ...
    if log_fn:
        log_fn(f"Output written: {output_path}  ({include_count} INCLUDE lines total)")
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        # --- Log area ---
        ttk.Label(frame, text="Log:").grid(row=6, column=0, sticky="w", padx=8)
        self._log = scrolledtext.ScrolledText(frame, height=18, state="disabled", wrap="word")
        self._log.grid(row=7, column=0, columnspan=3, sticky="nsew", padx=8, pady=4)
    ...
    def _run_generation(self, gfem, excel, search_dir, output):
        try:
            self._log("[INFO] Reading Excel...")
            case_ids = read_case_ids_from_excel(excel)
            self._log(f"[INFO] Found {len(case_ids)} unique case IDs (thermal excluded).")

            self._log(f"[INFO] Scanning directory: {search_dir}")
            files_by_id = find_bdf_files_for_ids(search_dir, case_ids, log_fn=self._log)

            found_count = sum(1 for v in files_by_id.values() if v)
            not_found = sorted(cid for cid, v in files_by_id.items() if not v)
            ...
            self._log(
                f"[INFO] {found_count}/{len(case_ids)} IDs matched files. "
                f"{len(not_found)} IDs had no match."
            )
